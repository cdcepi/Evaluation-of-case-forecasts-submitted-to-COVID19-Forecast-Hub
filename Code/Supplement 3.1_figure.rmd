...
## Change labels for counties
 rename_counts <-function(df) {   
   df <-df %>%
     mutate(pop_size_quant2=ifelse(pop_size_quant2==1, "County Forecasts: Size Quintile 1",
                                 ifelse(pop_size_quant2==2, "County Forecasts: Size Quintile 2",
                                        ifelse(pop_size_quant2==3, "County Forecasts: Size Quintile 3", 
                                               ifelse(pop_size_quant2==4, "County Forecasts: Size Quintile 4", "County forecasts: Size Quintile 5")))))
}
...
mean_WIS <-function() {
  ...
  WIS_counties <-WIS_all %>%
    ...
    distinct() %>%
    mutate(pop_size_quant2="County Forecasts: Size Quintile 5",
           model_reorder=fct_reorder(model, scaled_rel_skill))
  levels_model <-levels(WIS_counties$model_reorder)

  ## Non Large counties
  load(here("Data/WIS_all horizons_non large counties_all subs.rdata"))
  ...
  WIS_counties_other <-WIS_all %>%
    ...
    distinct() %>%
    rename_counts() %>%
    add_row(model="FRBSF_Wilson-Econometric", pop_size_quant2="County Forecasts: Size Quintile 1") %>% #,
    add_row(model="UMass-MechBayes", pop_size_quant2="County Forecasts: Size Quintile 1") %>% #, 
    add_row(model="UMass-MechBayes", pop_size_quant2="County Forecasts: Size Quintile 2") %>% #, 
    add_row(model="UMass-MechBayes", pop_size_quant2="County Forecasts: Size Quintile 3") %>% #, 
    add_row(model="UMass-MechBayes", pop_size_quant2="County Forecasts: Size Quintile 4") %>% 
...
cov.95 <-function() {
  ...
  load(here("Data/cases_non large counties_forecasts_for analysis.rdata"))
...
cov.95 <-cov.95() %>%
  ...
  ungroup() %>%
  mutate(key=pop_size_quant2) %>%
  add_row(model="FRBSF_Wilson-Econometric", key="County Forecasts: Size Quintile 1") %>% #, 
  add_row(model="UMass-MechBayes", key="County Forecasts: Size Quintile 1") %>% #, 
  add_row(model="UMass-MechBayes", key="County Forecasts: Size Quintile 2") %>% #, 
  add_row(model="UMass-MechBayes", key="County Forecasts: Size Quintile 3") %>% #, 
  add_row(model="UMass-MechBayes", key="County Forecasts: Size Quintile 4") %>%
  mutate(model_reorder=factor(model, levels_model))
