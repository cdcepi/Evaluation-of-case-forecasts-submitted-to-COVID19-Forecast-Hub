...
for(i in 1:length(team)){
  ...
  marg_means[[i]] <-as.data.frame(ggemmeans(model[[i]], terms=c("phase", "cases_log_std[-1.96:1.96]")) %>%
    rename(phase=x,
           cases=group))
...
FigS7 <-marg_means %>%
  ...
  geom_ribbon(aes(ymin=predicted_back_lo_rescale, ymax=predicted_back_up_rescale, fill=phase), alpha=0.35) +
...
```{r Fig S7, echo=FALSE, warning=F, message=F, fig.height=8, fig.width=11.5}

grid.draw(shift_legend(FigS7))
