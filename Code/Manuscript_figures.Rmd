...
load(here("Data/cases_nat_state_forecasts_for analysis_2022-08-18.Rdata"))
...
submissions <-function(df) {
  ...
  df <-df %>%
    filter(sub_date >= start_date & sub_date <= cut_date) %>% 
...
# Largest Counties
load(here("Data/cases_large counties_forecasts_for analysis_2022-08-18.Rdata"))
...
for(i in 1:length(team)){
  ...
  marg_means[[i]] <-ggemmeans(model[[i]], terms=c("phase")) %>% 
    rename(phase=x,
           cases=group) %>%
...
plot_grid(Fig7A, Fig7B, rel_heights  = c(0.5, 1), axis = "l",
          align="v", nrow = 2)
