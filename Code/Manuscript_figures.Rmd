...
### Need functions
source(here("Code/coverage.R")) #coverage
...
heatmap_format <-theme(axis.title.x = element_blank(), 
                       axis.title.y = element_blank(), 
                       axis.text=axis.text_standard,
                       legend.position="bottom", legend.title.align=0.5, legend.box.just = "center") 
...
source(here("Code/Reported cases.R")) #reported cases
obs_data <-rd_obs() %>%
...
Fig1.A <-ggplot() +
  ...
  labs(title = "A. Reported Cases Per 100K Population",
       y = "Incident COVID-19 Cases per 100K population") +
  theme_bw() +
...
load(here("Data/cases_nat_state_forecasts_for analysis.Rdata"))
...
Fig1.B <-dat_US_state %>%
  ...
  theme(axis.title.x = element_blank(), axis.text.y=element_text(angle=90, vjust=0.5, hjust=0.5),
        legend.title = element_blank(), legend.position = c(0.20, 0.82)) + 
  scale_y_continuous(label=comma, breaks=pretty_breaks(2))
...
mean_WIS <-function() {
  load(here("Data/WIS_all horizons_nat_state.rdata"))
...
WIS.heatmap <-mean_WIS %>%
  ...
  labs(fill="Relative Weighted Interval Score",
       title="B. Relative Weighted Interval Score") +
  heatmap_format + theme(axis.text.y=element_blank())
...
all_dat <-function(){
  ...
  load(here("Data/cases_nat_state_forecasts_2021-03-13.Rdata"))
  load(here("Data/cases_nat_state_forecasts_2022-03-21.Rdata"))
...
all_subs <-subs_nat %>%
  ...
  theme_bw() +
  labs(fill="% Weeks \nSubmitted",
       title="A. Percent of Weeks Submitted") +
  heatmap_format
...
# Largest Counties
load(here("Data/cases_large counties_forecasts_for analysis.Rdata"))
...
relWIS_co <-function(){
  ...
  load(here("Data/WIS_all horizons_large counties.Rdata"))
  ...
  load(here("Data/WIS_all horizons_non large counties_all.Rdata"))
  rel_WIS_co <-WIS_all %>%
    ungroup() %>%
...
### Figure 7 . Marginal mean WIS and 95% COverage

```{r echo=FALSE, warning=F, message=F}

load(here("Data/WIS_gee.Rdata")) # data
load(here("Data/gee_for_manuscript.rda")) 
...
for(i in 1:length(team)){
  ...
  marg_means[[i]] <-as.data.frame(ggemmeans(model[[i]], terms=c("phase")) %>% #, "cases_log_std[-2:2]")) %>%
    rename(phase=x,
           cases=group) 
    )
  
  marg_means[[i]] <-marg_means[[i]] %>%
...
Fig7A <-marg_means_int %>%
  ...
  labs( y="Marginal mean\n Weighted Interval Score", 
        title="A.") +
  theme_bw() +
  theme(axis.title.y=axis.title.y_standard, axis.text.y=axis.text_standard,
        axis.title.x = element_blank(),
...
Fig7B <-marg_means %>%
  ...
  theme_bw() +
  heatmap_format +
  theme(axis.text.x = element_blank(), #axis.text.x = element_text(angle = 45, hjust=1), 
...
load(here("Data/WIS_all horizons_nat_state.rdata")) # data
load("./Data/covid_epi_phases.rdata")
...
Fig7C <-WIS_all %>%
  ...
  group_by(phase, model) %>%
  summarise(cov=round(mean(coverage), 2)) %>%
  ggplot() +
  geom_tile(aes(model, phase, fill=cov)) +
  geom_text(aes(model, phase, label=cov), size=2.5) +
  scale_fill_gradient2(low = "white", mid = "#f7fcb9", high = "#31a354", midpoint = 0.75, na.value = "grey75",
...
```{r Fig7, echo=FALSE, warning=F, message=F, fig.height=11, fig.width=8.5}

plot_grid(Fig7A, Fig7B, Fig7C,
          rel_heights  = c(0.35, 0.50, 0.70), axis = "l",
          align="v", nrow = 3)

```

### Figure 8: % Prediction in Each Direction

```{r echo=FALSE, warning=F, message=F}

#levels <-levels[!levels %in% "COVIDhub-baseline"]
#
dat <-dat_US_state %>%
...
base <-dat_US_state %>%
  filter(grepl("baseline", model),
         quantile==0.5) %>%
  pivot_wider(names_from = c(type, quantile), values_from=value) %>%
  rename(quantile_0.50_base=quantile_0.5) %>%
  dplyr::select(-model, -sub_date, -forecast_date)

direct <-dat %>%
  left_join(., base, by=c("location", "target", "target_end_date")) %>%
  mutate(pred_class=as.factor(ifelse(quantile_0.25 > quantile_0.50_base, "increasing",
                      ifelse(quantile_0.75 < quantile_0.50_base, "decreasing", "stable/uncertain")))) %>%
  rename(date=target_end_date) %>%
  left_join(., phases, by=c("date", "location")) %>%
  drop_na() %>%
  mutate(model=as.factor(model), 
         model=factor(model, levels=levels),
         phase=as.factor(str_to_title(phase)),
         phase=factor(phase, levels=c("Nadir", "Decreasing", "Peak", "Increasing")))

team_phase_pred <-crossing(direct$model, direct$pred_class, direct$phase) %>%
  rename(model=1, 
         pred_class=2,
         phase=3) #%>%
  #filter(!grepl("baseline", model))
...
team_phase <-direct %>%
  ...
  count() %>%
  rename(phase_denom=n)

direct <-direct %>%
  group_by(model, phase, pred_class) %>%
  count(.) %>%
  ungroup() %>%
  right_join(., team_phase_pred, by=c("model", "phase", "pred_class")) %>%
  left_join(., team_phase, by=c("model", "phase")) %>%
  mutate(precent_pred=n/phase_denom, 
         precent_pred=ifelse(is.na(precent_pred), 0, precent_pred), 
         value_display=format(round(precent_pred, digits=2), nsmall = 2),
         value_display=ifelse(value_display=="  NA", NA, value_display))

Fig8A <-direct %>%
  filter(pred_class=="increasing") %>%
  ggplot(aes(model, phase)) +
  geom_tile(aes(fill=precent_pred)) +
  geom_text(aes(label=value_display), size=2.75) +
  scale_fill_gradient2(low = "#fdd0a2", high = "#e6550d",  na.value = "grey75",
                       name = stringr::str_wrap("Percent of forecasts with predicted increasing trajectory",
                                                 width = 35), 
                       guide = guide_colourbar(barheight = 1.5, title.position = "top")) +#, title.hjust=0.5)) +
  labs(y="", title="A.") + 
  theme_bw() +
  heatmap_format +
  theme(axis.text.x = element_blank(), 
        axis.ticks.y=element_blank(), 
        legend.position = "right", legend.direction = "horizontal", legend.title.align=0) 


Fig8B <-direct %>%
  filter(pred_class=="stable/uncertain") %>%
  ggplot(aes(model, phase)) +
  geom_tile(aes(fill=precent_pred)) +
  geom_text(aes(label=value_display), size=2.75) +
  scale_fill_gradient2(low = "#dadaeb", high = "#54278f",  na.value = "grey75",
                       name = stringr::str_wrap("Percent of forecasts with predicted stable or uncertain trajectory",
                                                 width = 35), 
                       guide = guide_colourbar(barheight = 1.5, title.position = "top")) +#, title.hjust=0.5)) +
  labs(y="", title="B.") + 
  theme_bw() +
...
Fig8C <-direct %>%
  filter(pred_class=="decreasing") %>%
  ggplot(aes(model, phase)) +
  geom_tile(aes(fill=precent_pred)) +
  geom_text(aes(label=value_display), size=2.75) +
  scale_fill_gradient2(low = "#a1d99b", high = "#006d2c", na.value = "grey75",
                       name = stringr::str_wrap("Percent of forecasts with predicted decreasing trajectory",
                                                 width = 35), 
                       guide = guide_colourbar(barheight = 1.5, title.position = "top")) +#, title.hjust=0.5)) +
  labs(y="", title="C.") + 
  theme_bw() +
...
```{r Fig9, echo=FALSE, warning=F, message=F, fig.height=8.5, fig.width=11}

plot_grid(Fig8A, Fig8B, Fig8C,
          rel_heights  = c(0.50, 0.50, 0.82), axis = "l",
          align="v", nrow = 3)
