...
### Estimate Relative WIS 
pairWIS_fun <-function(x, df) {
  
  df <-df %>%
    pairwise_comparison(., summarise_by = c("model"),
                        baseline = "COVIDhub-baseline") %>%
    dplyr::select(model, scaled_rel_skill) %>%
    distinct()
}
pairWIS_co_fun <-function(x, df) {
  
  df <-df %>%
    pairwise_comparison(., summarise_by = c("model", "pop_size_quant2"),
                        baseline = "COVIDhub-baseline") %>%
    dplyr::select(model, pop_size_quant2, scaled_rel_skill) %>%
    distinct()
}
