---
title: "Supplement 2: Team Specific GEEs  & Exploration of Revision Impact"
author: "VKL"
...
cases_all <-main_dat %>%
  left_join(., hx_dats_all, by=c("geo_value", "time_value")) %>%
  mutate(revision=ifelse(cases_now==cases_last, 0, 1)) %>%
  filter(revision==1) %>%
  dplyr::select(geo_value, wk, revision) %>%
  distinct()

## rename locations based on state names
states <-read.csv("C:/Users/oko8/OneDrive - CDC/Projects/covid19-forecast-hub/data-locations/locations.csv") %>%
...
## Read in data
load("C:/Users/oko8/OneDrive - CDC/COVID/Forecasts/Data/WIS_gee.Rdata")

WIS_rev <-WIS_gee %>%
   arrange(location_name, date) %>%
   left_join(., cases_all, by=c("location_name", "date")) %>%
   mutate(revision=ifelse(is.na(revision), 0, revision)) %>%
   group_by(model, location_name, target) %>%
   ungroup() %>%
   arrange(location_name, date) %>%
   filter(revision==0) %>%
   mutate(location_name=droplevels(location_name))
...
hx_dats_all %>%
  ...
  ungroup() %>%
  ggplot(.) +
  geom_line(aes(time_value, cases_last), color="#cb181d", alpha=0.55) + #as.factor(wk)),  alpha=0.65) +
  geom_line(main_dat, mapping=aes(time_value, cases_now), size=0.65) +
  facet_wrap(~ geo_value, scales="free_y") +
  scale_x_date(date_labels="%b-%y", breaks = hx_dats_all$wk, date_breaks ="3 months", date_minor_breaks ="1 month") +
...
```{r Fig S2.2, echo=FALSE, warning=F, message=F, fig.height=8.5, fig.width=8.5}

FigS2.2_updated <-FigS2.2 + theme(axis.text.x = element_blank())

plot_grid(FigS2.2_updated, Fig6B, rel_heights  = c(0.5, 1), axis = "l",
          align="v", nrow = 2)
